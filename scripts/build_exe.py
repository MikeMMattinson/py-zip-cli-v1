...
"""
Script:     build_exe.py
Purpose:    Builds versioned .exe from zipcli/main.py, updates README.txt with usage section,
            and organizes everything into versioned folders. Also creates a .zip and copies
            it to /mnt/zipcli (Samba share) for network distribution.
"""
...
# Step 3: Inject usage section and copy README and USAGE
if README.exists():
    if USAGE.exists():
    ...
    # Inject usage into README
    update_readme_usage(README, usage)

    # Copy updated README and USAGE into the dist folder
    shutil.copy(README, target_folder)
    print(f"📄 Copied updated README.txt to {target_folder}")

    if USAGE.exists():
        shutil.copy(USAGE, target_folder)
        print(f"📄 Copied USAGE.txt to {target_folder}")
else:
...
if platform.system() == "Windows":
    win_share_path = Path(r"\\oscar\Zip-CLI")
